...
Each target is reduced to one median gaze and context record. A batch is scored before any record from that batch trains a candidate. A failed, already reported batch may enter the candidate coreset. The next batch remains unseen. The final successful or terminal batch never becomes training data. If later adaptation regresses before the target limit, Gazeebo retains the best non-regressing model measured during the invocation and excludes that model's holdout batch from its training records.
